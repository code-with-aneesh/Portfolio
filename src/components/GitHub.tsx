...
export function GitHub() {
    ...
    useEffect(() => {
        fetch("https://api.github.com/users/code-with-aneesh")
            .then((response) => response.json())
            .then((data: UserProfile) => setUser(data));

        fetch("https://api.github.com/users/code-with-aneesh/repos?sort=pushed&direction=desc&per_page=3")
            .then((response) => response.json())
            .then((data) =>
                setRepos(
                    data.map((repo: any) => ({
